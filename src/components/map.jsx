...
const CampusMap = () => {
    ...
    useEffect(() => {
        ...
        buildings.current = [
            {
                name: 'Main Building',
                path: new Path2D(),
                bounds: { x: 200, y: 50, width: 400, height: 100 }
            },
            {
                name: 'MBA',
                path: new Path2D(),
            ...
            },
            {
                name: 'Gate',
                path: new Path2D(),
                bounds: { x: 600, y: 170, width: 50, height: 100 }
            }
        ];
        ...
        const draw = () => {
            ...
            buildings.current.forEach(building => {
                ...
                // Apply blue color only for specific buildings
                const isSpecialBuilding = ['Library Block', 'Basketball Court', 'C Block'].includes(building.name);
                const fillColor = isSpecialBuilding
                    ? (hoveredBuilding === building.name ? 'rgba(100, 149, 237, 0.5)' : 'rgba(100, 149, 237, 0.3)') // Blue for these buildings
                    : 'rgba(200, 200, 200, 0.1)'; // Light gray for others
    ...
    }, [router, hoveredBuilding]);

    return (
        <>
            <h1 id='map' className='text-4xl font-bold text-center mt-24'>Overview of the campus</h1>
